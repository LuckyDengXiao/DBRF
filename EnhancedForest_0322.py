...
class EnhancedForest():
    ...
    def predict_proba(self, X):
        ...
        if self.isLRStacker and self.log_model != None:
    ...
    def get_forest_leaf_index(self, clf, X_valid, y_valid):
        ...
        for index, tree in enumerate(clf):
            ...
            # node_id_score = 2*node_id_count*node_id_lift/(node_id_count+node_id_lift)
    ...
    def fit(self, X, y, X_test, real_y, verbose=True, feval=None, 
        ...
        if self.isLRStacker:
            X_, X_dev, y_, y_dev = train_test_split(X, y, test_size=0.35, random_state=500)
        else:
            X_, y_ = X, y
        ...
        est_leaf_index = []
        for i, (train_index, test_index) in enumerate(sss.split(X_, y_)):
            # split_obj = tuple(sss.split(X, y))
            # train_index, test_index = split_obj[0]
            X_train, X_valid = X_[train_index], X_[test_index]
            ...
            clf = RandomForestClassifier(max_depth=max_depth, random_state=random_state, 
                                         min_samples_leaf=min_samples_leaf, n_estimators=50, n_jobs=-1,
                                         #max_leaf_nodes=100,
    #                                  n_estimators=2, n_jobs=8, oob_score=True, verbose=1, boostrap=False,
        ...
        if self.isLRStacker:
            y_preds = np.zeros((X_dev.shape[0], len(self.estimator)))
            for i, clf in enumerate(self.estimator):
                y_pred = clf.predict_proba(X_dev)[:,1]
                y_preds[:, i] = y_pred

            log_model = LogisticRegression()
            log_model.fit(y_preds, y_dev)
